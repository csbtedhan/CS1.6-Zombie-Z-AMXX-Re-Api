...
new g_HadWpn
new Float:c_gren_damage[2]
new Float:c_gren_radius[2]
new c_projectile_num
...
public load_config()
{
	if(!file_exists("script/custom_item/weapon_anyagren.txt"))
	{
		delete_file("script/custom_item/weapon_anyagren.txt")
		new File = fopen("script/custom_item/weapon_anyagren.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "GRENADE_RADIUS	350.0^n")
		fprintf(File, "GRENADE_DAMAGE	100.0^n")
		fprintf(File, "PROJECTILE_RADIUS	250.0^n")
		fprintf(File, "PROJECTILE_DAMAGE	500.0^n")
		fprintf(File, "PROJECTILE_COUNT	13^n")
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_anyagren.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		if(equal(key, "GRENADE_RADIUS")) c_gren_radius[0] = str_to_float(value)
		else if(equal(key, "GRENADE_DAMAGE")) c_gren_damage[0] = str_to_float(value)
		else if(equal(key, "PROJECTILE_RADIUS")) c_gren_radius[1] = str_to_float(value)
		else if(equal(key, "PROJECTILE_DAMAGE")) c_gren_damage[1] = str_to_float(value)
		else if(equal(key, "PROJECTILE_COUNT")) c_projectile_num = str_to_num(value)
	}
	fclose(file)
}
public plugin_init()
...
public GrenadeExplode(Ent)
{
	...
	while((pEnt = engfunc(EngFunc_FindEntityInSphere, pEnt, vecOrigin, c_gren_radius[0])) != 0 )
	{
		...
		flDamage = c_gren_damage[0] - floatmul(c_gren_damage[0], floatdiv(flDist, c_gren_radius[0]));
		
		if(isBreakable(pEnt) && flDamage <= 0.0) flDamage = floatdiv(c_gren_damage[0], 2.0)
...
public Spit(Float:vOrigin[3], Attacker)
{
	...
	for(new x=0; x<c_projectile_num;x++)
	{
...
public SpitImpact(Ent, Id)
{
	...
	while((pEnt = engfunc(EngFunc_FindEntityInSphere, pEnt, vOrigin, c_gren_radius[1])) != 0 )
	{
		if(!pev_valid(pEnt) || pEnt == pevAttacker)
			continue;
		...
		flDamage = c_gren_damage[1] - floatmul(c_gren_damage[1], floatdiv(flDist, c_gren_radius[1]));
		
		if(isBreakable(pEnt) && flDamage <= 0.0) flDamage = floatdiv(c_gren_damage[1], 2.0)
