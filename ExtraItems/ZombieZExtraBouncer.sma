...
new c_wpn_clip = 25
new c_wpn_damage = 80
new c_wpn_shotcount = 7
new Float:c_wpn_spreadsize = 150.0
new c_wpn_shotspeed = 3500
new c_wpn_bounce = 2
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_bouncer.txt"))
	{
		delete_file("script/custom_item/weapon_bouncer.txt")
		new File = fopen("script/custom_item/weapon_bouncer.txt", "a")
		...
		fprintf(File, "SPREAD_SIZE %.1f^n", c_wpn_spreadsize)
		fprintf(File, "BULLET_VELOCITY %i^n", c_wpn_shotspeed)
		fprintf(File, "BASE_DAMAGE %i^n", c_wpn_damage)
		fprintf(File, "BOUNCE_REPEAT %i^n", c_wpn_bounce)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_bouncer.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "SPREAD_SIZE")) c_wpn_spreadsize = str_to_float(value)
		else if(equal(key, "BULLET_VELOCITY")) c_wpn_shotspeed = str_to_num(value)
		else if(equal(key, "BOUNCE_REPEAT")) c_wpn_bounce = str_to_num(value)
	}
	fclose(file)
}
...
public GiveBouncer(id)
{
	...
	if(pev_valid(iWpn) && get_pdata_int(iWpn, 43) == CSW_M3)
		set_pdata_int(iWpn, 51, c_wpn_clip, 4);
}
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if((CurButton & IN_RELOAD) && get_pdata_int(Ent, 51, 4) >= c_wpn_clip)
	{
		CurButton &= ~IN_RELOAD
...
public Shoot_Bouncer(id)
{
	...
	static iEnt
	for( new x = 0; x < c_wpn_shotcount ; x ++ )
	{
		...
		set_pev(iEnt, pev_iuser1, c_wpn_bounce)
		set_pev(iEnt, pev_iuser2, 75811)
		
		engfunc(EngFunc_SetModel, iEnt, S_MODEL);
		
		velocity_by_aim(id, c_wpn_shotspeed, vecVelocity);
		vecVelocity[0] += random_float(-c_wpn_spreadsize, c_wpn_spreadsize)
		vecVelocity[1] += random_float(-c_wpn_spreadsize, c_wpn_spreadsize)
		vecVelocity[2] += random_float(-c_wpn_spreadsize, c_wpn_spreadsize)
...
public Bullet_Impact(iEnt, pEnt)
{
	...
	if(flFraction < 1.0)
	{
		if(is_user_alive(pEnt))
		{
			...
			{
				rg_multidmg_clear()
				ExecuteHamB(Ham_TraceAttack, pEnt, pevAttacker, float(c_wpn_damage), vDir, tr, DMG_BULLET);
				rg_multidmg_apply(pevAttacker, pevAttacker)
			}
		} else {
			if(pev_valid(pEnt) && pev(pEnt, pev_takedamage) > 0.0)
				ExecuteHamB(Ham_TakeDamage, pEnt, iEnt, pevAttacker, float(c_wpn_damage), DMG_CLUB);
		}
	}
	free_tr2(tr)
