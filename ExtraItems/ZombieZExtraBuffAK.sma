...
new c_wpn_bdamage = 800
new c_wpn_bradius = 150
new c_wpn_bvelo = 1850
new Float:c_wpn_bspeed = 0.6
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_buffak.txt"))
	{
		delete_file("script/custom_item/weapon_buffak.txt")
		new File = fopen("script/custom_item/weapon_buffak.txt", "a")
		...
		fprintf(File, "PROJECTILE_RATE %.1f^n", c_wpn_bspeed)
		fprintf(File, "PROJECTILE_DAMAGE %i^n", c_wpn_bdamage)
		fprintf(File, "PROJECTILE_RADIUS %i^n", c_wpn_bradius)
		fprintf(File, "PROJECTILE_VELOCITY %i^n", c_wpn_bvelo)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_buffak.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "PROJECTILE_DAMAGE")) c_wpn_bdamage = str_to_num(value)
		else if(equal(key, "PROJECTILE_RADIUS")) c_wpn_bradius = str_to_num(value)
		else if(equal(key, "PROJECTILE_VELOCITY")) c_wpn_bvelo = str_to_num(value)
		else if(equal(key, "PROJECTILE_RATE")) c_wpn_bspeed = str_to_float(value)
	}
	fclose(file)
}
public plugin_init()
...
public ShotBall(id)
{
	...
	velocity_by_aim(id, c_wpn_bvelo, Velocity)
...
public BallImpact(Ent, id)
{
	...
	Damaging(pevAttacker, Ent, pOrigin, float(c_wpn_bradius), float(c_wpn_bdamage));
