...
load_config()
{
	if(!file_exists("script/custom_item/weapon_chaingren.txt"))
	{
		delete_file("script/custom_item/weapon_chaingren.txt")
		new File = fopen("script/custom_item/weapon_chaingren.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "GRENADE_RADIUS %f^n", c_grenade_radius)
		fprintf(File, "GRENADE_DAMAGE %f^n", c_grenade_damage)
		fprintf(File, "GRENADE_REPEAT %i^n", c_grenade_chainnum)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_chaingren.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "GRENADE_REPEAT")) c_grenade_chainnum = str_to_num(value)
	}
	fclose(file)
}
public plugin_init()
...
public fw_CSZB_HumanNade_Pre(Ent, Id)
{
	...
	set_pev(Ent, pev_iuser2, c_grenade_chainnum);
...
public GrenadeExplode(Ent)
{
	...
	while((pEnt = engfunc(EngFunc_FindEntityInSphere, pEnt, vecOrigin, c_grenade_radius)) != 0 )
	{
		...
		flDamage = c_grenade_damage - floatmul(c_grenade_damage, floatdiv(flDist, c_grenade_radius));
