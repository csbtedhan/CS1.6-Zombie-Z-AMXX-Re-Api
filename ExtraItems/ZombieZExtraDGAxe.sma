...
new Float:c_slash_damage[2] = {300.0, 350.0}
new Float:c_slash_range
new Float:c_slash_kb
#define SLASH_ANGLE			120.0
#define SLASH_DAMAGE			random_float(c_slash_damage[0],c_slash_damage[1])
#define SLASH_RANGE			130.0
#define SLASH_KNOCKBACK 		1.0		

//EXPLO1
new Float:c_explo1_damage[2] = {600.0, 650.0}
new Float:c_explo1_range
new Float:c_explo1_kb
#define EXPLO1_DAMAGE			random_float(c_explo1_damage[0] ,c_explo1_damage[1] )	
#define EXPLO1_RANGE			160.0
#define EXPLO1_KNOCKBACK 		10.0				

//EXPLO2
new Float:c_explo2_damage[2] = {600.0, 650.0}
new Float:c_explo2_range
new Float:c_explo2_kb
#define EXPLO2_DAMAGE			random_float(c_explo2_damage[0] ,c_explo2_damage[1] )		
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_dgaxe.txt"))
	{
		delete_file("script/custom_item/weapon_dgaxe.txt")
		new File = fopen("script/custom_item/weapon_dgaxe.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "SLASH_ANGLE %i^n", floatround(SLASH_ANGLE))
		fprintf(File, "SLASH_DAMAGE %i %i^n", floatround(c_slash_damage[0]), floatround(c_slash_damage[1]))
		fprintf(File, "SLASH_RANGE %i^n", floatround(SLASH_RANGE))
		fprintf(File, "SLASH_KNOCKBACK %i^n^n", floatround(SLASH_KNOCKBACK))
		
		fprintf(File, "SUMMON_DAMAGE %i %i^n", floatround(c_explo1_damage[0]), floatround(c_explo1_damage[1]))
		fprintf(File, "SUMMON_RANGE %i^n", floatround(EXPLO1_RANGE))
		fprintf(File, "SUMMON_KNOCKBACK %i^n^n", floatround(EXPLO1_KNOCKBACK))
		
		fprintf(File, "EXPLOSION_DAMAGE %i %i^n", floatround(c_explo2_damage[0]), floatround(c_explo2_damage[1]))
		fprintf(File, "EXPLOSION_RANGE %i^n", floatround(EXPLO2_RANGE))
		fprintf(File, "EXPLOSION_KNOCKBACK %i^n^n", floatround(EXPLO2_KNOCKBACK))
		
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64], extra[64]
	file = fopen("script/custom_item/weapon_dgaxe.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		if(equal(key, "SLASH_ANGLE")) c_slash_angle = str_to_float(value)
		else if(equal(key, "SLASH_DAMAGE")) 
		{
			c_slash_damage[0] = str_to_float(value);
			c_slash_damage[1] = str_to_float(extra);
		}
		...
		else if(equal(key, "SUMMON_DAMAGE")) 
		{
			c_explo1_damage[0] = str_to_float(value); 
			c_explo1_damage[1] = str_to_float(extra);
		}
		else if(equal(key, "SUMMON_RANGE")) c_explo1_range = str_to_float(value)
		else if(equal(key, "SUMMON_KNOCKBACK")) c_explo1_kb = str_to_float(value)
		
		else if(equal(key, "EXPLOSION_DAMAGE"))
		{
			c_explo2_damage[0] = str_to_float(value); 
			c_explo2_damage[1] = str_to_float(extra);
		}
		else if(equal(key, "EXPLOSION_RANGE")) c_explo2_range = str_to_float(value)
		else if(equal(key, "EXPLOSION_KNOCKBACK")) c_explo2_kb = str_to_float(value)
...
public WeaponSkill(id, iEnt, iButton)
{
	...
	if (g_combo[id] == 2 && get_pdata_float(id, 83) <= 0.0 && iState == 1)
	{
		...
		new iHitResult = KnifeAttack_Main(id, 0, c_slash_range, c_slash_angle, SLASH_DAMAGE, c_slash_kb)
		switch (iHitResult)
		{
		...
		if(!iMode)
		{
			if(fAnim && fAnim < get_gametime())
			{
				...
				if(iAnim == 1)
				{
					...
					Do_Damage(ent, iOwner, vVicOrig, c_explo1_range, EXPLO1_DAMAGE, c_explo1_kb)
				}
			}
			...
			if(fAnim && fAnim < get_gametime())
			{
				if(iAnim <= 2)
				{
					...
					Do_Damage(ent, iOwner, vOrig, c_explo2_range, EXPLO2_DAMAGE, c_explo2_kb)
