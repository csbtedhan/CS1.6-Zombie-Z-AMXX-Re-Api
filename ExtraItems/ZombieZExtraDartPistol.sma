...
new c_wpn_radius = 170
new c_wpn_bvelo = 2000
new c_wpn_pronum = 6
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_dartpistol.txt"))
	{
		delete_file("script/custom_item/weapon_dartpistol.txt")
		new File = fopen("script/custom_item/weapon_dartpistol.txt", "a")
		...
		fprintf(File, "PROJECTILE_BOLT_COUNT %i^n", c_wpn_pronum)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_dartpistol.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "PROJECTILE_BOLT_COUNT")) c_wpn_pronum = str_to_num(value)
	}
	fclose(file)
}
public plugin_init()
...
public ShootDart(id)
{
	...
	set_pev(pEnt, pev_iuser1, c_wpn_pronum)
	
	velocity_by_aim(id, c_wpn_bvelo, vVelocity)
...
stock DoElectricity(Float:vOrigin[3], pevAttacker, pevInflictor)
{
	...
	for(new id = 0; id < get_maxplayers(); id ++)
	{
		...
		if(!isClearLine(pOrigin, vOrigin) || vector_distance(vOrigin, pOrigin) > float(c_wpn_radius))
			continue;
		...
		ExecuteHamB(Ham_TakeDamage, id, pevInflictor, pevAttacker, float(c_wpn_damage), DMG_CLUB) 
	}
}
