...
new c_wpn_damage = 120
new Float:c_wpn_speed = 0.075
new Float:c_wpn_spreadmult = 0.0035
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_ethereal.txt"))
	{
		delete_file("script/custom_item/weapon_ethereal.txt")
		new File = fopen("script/custom_item/weapon_ethereal.txt", "a")
		...
		fprintf(File, "ACCURACY %f^n", c_wpn_spreadmult)
		fprintf(File, "FIRE_RATE %f^n", c_wpn_speed)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_ethereal.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "ACCURACY")) c_wpn_spreadmult = str_to_float(value)
...
public RG_FireBullets3(iAttacker, Float:vecSrc[3], Float:vecDirShooting[3], Float:vecSpread, Float:flDistance, iPenetration, iBulletType, iDamage, Float:flRangeModifier, pevAttacker, bool:bPistol, shared_rand)
{
	...
	SetHookChainArg(4, ATYPE_FLOAT, c_wpn_spreadmult)
