...
#define V_MODEL "models/zbz/wpn/v_ethereal2.mdl"
#define P_MODEL "models/zbz/wpn/p_primary.mdl"
#define W_MODEL "models/zbz/wpn/w_primary.mdl"


#define CSW_ETHEREAL CSW_AK47
#define weapon_ethereal "weapon_ak47"
...
new const WeaponSounds[] = "weapons/ethereal-1.wav"
...
public plugin_init()
{
	...
	RegisterHam(Ham_Weapon_PrimaryAttack, weapon_ethereal, "fw_PrimaryAttack");
	RegisterHam(Ham_Weapon_PrimaryAttack, weapon_ethereal, "fw_PrimaryAttack_Post", 1);
	RegisterHam(Ham_Item_Deploy, weapon_ethereal, "fw_Item_Deploy_Post", 1);
	RegisterHam(Ham_Item_Holster, weapon_ethereal, "fw_Item_Holster");
	...
	cs_register_custom_item("Ethereal", 3500, 4, "GiveBuffM4", "ethereal")
}
...
public GiveBuffM4(id)
{
	...
	if(pev_valid(Ent)) cs_set_weapon_ammo(Ent, 40)
}
...
{
	if(!is_user_connected(iAttacker) || !Get_BitVar(g_HadWpn, iAttacker) || get_user_weapon(iAttacker) != CSW_ETHEREAL) 
		return HC_CONTINUE;
	
	if(Get_BitVar(g_Attack, iAttacker))
		SetHookChainArg(8, ATYPE_INTEGER, 120)
...
public fw_TraceAttack(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	if(!is_user_connected(Attacker))
		return HAM_IGNORED	
	if(get_user_weapon(Attacker) != CSW_ETHEREAL || !Get_BitVar(g_HadWpn, Attacker))
		return HAM_IGNORED
...
public fw_SetModel(entity, model[])
{
	...
	{
		static weapon; weapon = find_ent_by_owner(-1, weapon_ethereal, entity)
		if(Get_BitVar(g_HadWpn, iOwner) && pev_valid(weapon))
		{
			set_pev(weapon, pev_impulse, 2891) 
...
{
	if ( !pev_valid( Ent ) || pev( Ent , pev_iuser1 ) != 76254 )
		return HAM_IGNORED;
	...
	set_pev( Ent , pev_renderamt , floatmin(225.0, floatmax(0.0, (pev(Ent, pev_dmgtime)-get_gametime())*225.0)) );
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	if(!is_user_alive(id))
		return FMRES_IGNORED	
	if(get_user_weapon(id) == CSW_ETHEREAL && Get_BitVar(g_HadWpn, id))
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (!is_user_connected(invoker))
		return FMRES_IGNORED	
	if(get_user_weapon(invoker) != CSW_ETHEREAL || !Get_BitVar(g_HadWpn, invoker))
		return FMRES_IGNORED
	if(eventid != g_Event)
		return FMRES_IGNORED
...
stock Stock_DrawBeam(Float:vStart[3], Float:vEnd[3])
{
	...
	set_pev(iBeamEntity, pev_rendercolor, Float: { 50.0 , 50.0 , 255.0 } );
	set_pev(iBeamEntity, pev_renderamt, 225.0);
	set_pev(iBeamEntity, pev_iuser1, 76254);
	...
	set_pev(iBeamEntity, pev_nextthink, get_gametime() + 0.1);
