...
new c_wpn_charge = 15
new Float:c_wpn_firedamage = 250.0
new Float:c_wpn_fireradius = 210.0
new Float:c_wpn_explodamage = 2250.0
new Float:c_wpn_exploradius = 350.0
new Float:c_wpn_fireduration = 6.0
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_gatlingex.txt"))
	{
		delete_file("script/custom_item/weapon_gatlingex.txt")
		new File = fopen("script/custom_item/weapon_gatlingex.txt", "a")
		...
		fprintf(File, "CHARGE_AMOUNT %i^n", c_wpn_charge)
		...
		fprintf(File, "EXPLOSION_DAMAGE %.1f^n", c_wpn_explodamage)
		fprintf(File, "EXPLOSION_RADIUS %.1f^n", c_wpn_exploradius)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_gatlingex.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "CHARGE_AMOUNT")) c_wpn_charge = str_to_num(value)
		...
		else if(equal(key, "EXPLOSION_DAMAGE")) c_wpn_explodamage = str_to_float(value)
		else if(equal(key, "EXPLOSION_RADIUS")) c_wpn_exploradius = str_to_float(value)
	}
	fclose(file)
}
public plugin_init()
...
public RG_FireBuckShots(id, cShots, Float:vecSrc[3], Float:vecDirShooting[3], Float:vecSpread[3], Float:flDistance, iTracerFreq, iDamage, iAttacker)
{
	if(!is_user_connected(iAttacker) || get_user_weapon(iAttacker) != CSW_USAS) 
		return;
...
public fw_PrimaryAttack_Post(Ent)
{
	...
	{
		if(pev(Ent, pev_iuser2) < c_wpn_maxfire)
		{
			set_pev(Ent, pev_iuser1, pev(Ent, pev_iuser1) + 1)
			if(pev(Ent, pev_iuser1) >= c_wpn_charge)
			{
...
public ShootFireBall(id)
{
	...
	set_pev(pEnt, pev_dmgtime, get_gametime() + c_wpn_fireduration)
...
public FireBallThink(Ent)
{
	...
	DoRadiusDamage(vOrigin, c_wpn_firedamage, c_wpn_fireradius, pevOwner, Ent)
	
	if(pev(Ent, pev_dmgtime) <= get_gametime())
	{
		DoRadiusDamage(vOrigin, c_wpn_explodamage, c_wpn_exploradius, pevOwner, Ent)
		FireBallExplode(Ent)
		return;
	}
