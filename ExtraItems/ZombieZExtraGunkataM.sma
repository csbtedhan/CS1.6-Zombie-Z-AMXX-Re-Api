...
// Data Config
#define P_GUNKATA_M "models/zbz/wpn/p_gunkatam.mdl"
#define V_GUNKATA "models/zbz/wpn/v_gunkatamx.mdl"
#define V_GUNKATA2 "models/zbz/wpn/v_gunkatamx2.mdl"
...
#define SKILL_DAMAGE 90.0
#define SKILL_RANGE 150.0
#define SKILL_ANGLE 179.9
#define SKILL_KNOCKBACK 35.0
...
public plugin_init()
{
	...
	cs_register_custom_item("Dual Berreta Windrider", 2100, 1, "Get_Base", "gunkatam")
}
...
public plugin_natives(){
	register_native("give_weapon_gk","native_give_weapon_gk",1)
}
public native_give_weapon_gk(id){
	Get_Base(id)
}
...
public WE_GUNKATA(id, Ent, iButton)
{
	...
	if((iButton & IN_ATTACK2) && g_special[id] != 3)
	{
		...
		{
			set_pdata_float(Ent, 47, SKILL_DMGROF, 4)
			
			new iHit = KnifeAttack2(id, SKILL_RANGE, SKILL_DAMAGE, 4.0)
...
public Fw_AddToFullPack_Post(esState, iE, iEnt, iHost, iHostFlags, iPlayer, pSet)
{
	...
	if (equal(classname,"gunkatamprojectile") && pev(iEnt, pev_iuser1) != 1)
	{
...
public Fw_DBGEnt_Think(iEnt)
{
	...
	if(!equal(Classname, "gunkatamprojectile"))
		return
...
{
	static iMuz
	iMuz = Stock_CreateEntityBase(id, "env_sprite", MOVETYPE_FOLLOW, MODEL_MF, "dbgm_mf", SOLID_NOT,0.15,Float:{0.0,0.0,0.0},Float:{0.0,0.0,0.0})
...
public Gunkata_Blast(id)
{
	...
	new iEnt = Stock_CreateEntityBase(id, "info_target", 0, MODEL_BLAST, "gunkatamprojectile", SOLID_NOT,0.3,origin,Float:{0.0,0.0,0.0})
...
public DBG_Specials(ent, id, seq, Float:TimeRemove)
{
	...
	new pEntity = Stock_CreateEntityBase(id, "info_target", MOVETYPE_FLY, V_GUNKATA2, "gunkatamprojectile", SOLID_NOT,0.001,vecOrigin,vecAngle)
...
public DBG_Shadow(id, anim)
{
	...
	new pEntity = Stock_CreateEntityBase(id, "info_target", MOVETYPE_FLY, MODEL_SHADOW, "gunkatamprojectile", 0, 0.01, Origin, Angles)
