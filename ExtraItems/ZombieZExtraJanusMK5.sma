...
new c_wpn_bdamage = 210
new c_wpn_charge = 40
new Float:c_wpn_recoil = 0.3
new Float:c_wpn_btime = 7.0
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_janusmk5.txt"))
	{
		delete_file("script/custom_item/weapon_janusmk5.txt")
		new File = fopen("script/custom_item/weapon_janusmk5.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "CLIP_SIZE %i^n", c_wpn_clip)
		fprintf(File, "BASE_DAMAGE %i^n", c_wpn_damage)
		fprintf(File, "SKILL_DAMAGE %i^n", c_wpn_bdamage)
		fprintf(File, "PENETRATION %i^n", c_wpn_penetrate)
		fprintf(File, "RECOIL %f^n", c_wpn_recoil)
		fprintf(File, "SKILL_DURATION %i^n", floatround(c_wpn_btime))
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_janusmk5.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "SKILL_DAMAGE")) c_wpn_bdamage = str_to_num(value)
		...
		else if(equal(key, "SKILL_DURATION")) c_wpn_btime = str_to_float(value)
	}
	fclose(file)
}
...
public RG_FireBullets3(iAttacker, Float:vecSrc[3], Float:vecDirShooting[3], Float:vecSpread, Float:flDistance, iPenetration, iBulletType, iDamage, Float:flRangeModifier, pevAttacker, bool:bPistol, shared_rand)
{
	...
	SetHookChainArg(8, ATYPE_INTEGER, pev(Ent, pev_iuser2) == 1 ? c_wpn_bdamage : c_wpn_damage)
...
{	
	...
	if(is_user_connected(iVictim) && is_user_alive(iVictim) && cs_zb_is_zombie(iVictim))
	{
		if(pev(Ent, pev_iuser2)!=1)
		{
			...
			if(pev(Ent, pev_iuser3)==c_wpn_charge)
...
public fw_PrimaryAttack_Post(Ent)
{
	...
	if(get_pdata_int(Ent, 51, 4) > 0 && pev(Ent, pev_iuser2))
	{
		...
		xs_vec_mul_scalar(vPunch, c_wpn_recoil, vPunch);
...
public fw_Item_PostFrame(ent)
{
	...
	if(get_user_button(id) & IN_ATTACK2 && get_pdata_float(ent, 47, 4) <= 0.0 && pev(ent, pev_iuser3) >= c_wpn_charge && pev(ent, pev_iuser2) == 0)
	{
		Set_WeaponAnim(id, 5);
		...
		set_pev(ent, pev_dmgtime, get_gametime() + c_wpn_btime)
	}	
