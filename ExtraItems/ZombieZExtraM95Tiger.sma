...
new c_wpn_penetrate, c_wpn_damage, c_wpn_clip, Float:c_wpn_speed, Float:c_wpn_recoil, Float:c_wpn_reloadtime
new c_tiger_damage, c_tiger_range, c_net_ammo, c_tiger_charge
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_m95tiger.txt"))
	{
		delete_file("script/custom_item/weapon_m95tiger.txt")
		new File = fopen("script/custom_item/weapon_m95tiger.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "CLIP_SIZE %i^n", CLIP)
		fprintf(File, "BASE_DAMAGE %i^n", DAMAGE)
		fprintf(File, "PENETRATION %i^n", PENETRATION)
		fprintf(File, "FIRE_RATE %f^n", SPEED)
		fprintf(File, "RECOIL %f^n", RECOIL)
		fprintf(File, "RELOAD_TIME %f^n^n", RELOAD_TIME)
		
		fprintf(File, "TIGER_DAMAGE %i^n", floatround(DAMAGE_TIGER))
		fprintf(File, "TIGER_RANGE %i^n", floatround(RANGE_TIGER))
		fprintf(File, "NET_AMMO %i^n", AMMO_NET)
		fprintf(File, "CHARGE %i^n", HIT_COUNT)
		
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_m95tiger.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "TIGER_DAMAGE")) c_tiger_damage= str_to_num(value)
		else if(equal(key, "CHARGE")) c_tiger_charge = str_to_num(value)
		else if(equal(key, "TIGER_RANGE")) c_tiger_range = str_to_num(value)
		else if(equal(key, "NET_AMMO")) c_net_ammo = str_to_num(value)
	}
	fclose(file)
}
...
public Get_Base(id)
{
	...
	if(pev_valid(Ent)) 
	{
		cs_set_weapon_ammo(Ent, c_wpn_clip)
...
public WE_M95TIGER(id,iEnt,iClip, bpammo,iButton)
{
	...
	{
		if(g_HitCount[id] >= c_tiger_charge)
		{
	...
	if((iButton & IN_ATTACK) && (iButton & IN_ATTACK2) && g_HitCount[id] >= c_tiger_charge) // hold LMB and RMB while trigger to summon tiger
	{
		Shoot_Net(id, 1)
		g_HitCount[id] = 0
...
{
	if(!is_user_connected(iAttacker) || !Get_BitVar(g_Had_Base, iAttacker) || get_user_weapon(iAttacker) != CSW_M95TIGER) 
		return HC_CONTINUE;
...
public fw_TraceAttack_Player(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	...
	if(is_user_alive(Victim) && can_damage(Victim, Attacker) && g_Attacking[Attacker] && g_HitCount[Attacker] < c_tiger_charge)
	{
		g_HitCount[Attacker]++
		
		static Wpn; Wpn = get_pdata_cbase(Attacker, 373)
		if(pev_valid(Wpn) && g_HitCount[Attacker] >= c_tiger_charge) 
		{
...
public HamF_InfoTarget_Touch(iEnt, iPtd)
{
	...
	if(!iEntID)
	{
		...
		while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vecOri, RANGE_TIGER)) != 0)
		{
			...
			Stock_RadiusDamage(vOrigin, iEnt, iOwner, float(c_tiger_damage), float(c_tiger_range), 1.0, DMG_PARALYZE)
