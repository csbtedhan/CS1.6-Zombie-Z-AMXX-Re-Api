...
#define v_model "models/zbz/wpn/v_magicknife.mdl"
#define p_model "models/zbz/wpn/p_magicknife.mdl"
...
//MAGIC
new Float:c_mg_explo_dmg
new Float:c_mg_slash_dmg
new Float:c_mg_radius

#define MAGIC_EXPLOSION_DAMAGE		950.0
#define MAGIC_SLASH_DAMAGE		550.0
#define MAGIC_RADIUS			650.0
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_magicknife.txt"))
	{
		delete_file("script/custom_item/weapon_magicknife.txt")
		new File = fopen("script/custom_item/weapon_magicknife.txt", "a")
		...
		fprintf(File, "MAGIC_SLASH_DAMAGE %i^n", floatround(MAGIC_SLASH_DAMAGE))
		fprintf(File, "MAGIC_EXPLOSION_DAMAGE %i^n^n", floatround(MAGIC_EXPLOSION_DAMAGE))
		fprintf(File, "MAGIC_RADIUS %i^n^n", floatround(MAGIC_RADIUS))
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64], extra[64]
	file = fopen("script/custom_item/weapon_magicknife.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "MAGIC_SLASH_DAMAGE")) c_mg_slash_dmg = str_to_float(value)
		else if(equal(key, "MAGIC_EXPLOSION_DAMAGE")) c_mg_explo_dmg = str_to_float(value)
		else if(equal(key, "MAGIC_RADIUS")) c_mg_radius = str_to_float(value)
	}
	fclose(file)
}
...
public SpecialHandler(id, iEnt, iButton)
{
	...
	{
		new Float:Damage[4]
		Damage[0] = c_slash_range
		Damage[1] = c_slash_angle
		Damage[2] = c_slash_damage
		Damage[3] = c_slash_kb
...
public MagicInit(id)
{
	...
	for(new x = 0; x < get_maxplayers(); x ++ )
	{
		...
		if(isClearLine(pOrigin, vOrigin) && vector_distance(pOrigin, vOrigin) <= c_mg_radius  && !isLocked(x, id))
			MakeMagic(id, x)
	}
}
...
public MagicThink(Ent)
{
	...
	if(iStat == 1) ShootTraceAttack(pevAttacker, vOrigin, c_mg_slash_dmg)
	else if(iStat == 2)
	{
		ExecuteHamB(Ham_TakeDamage, iVictim, pevAttacker, pevAttacker, c_mg_explo_dmg, DMG_CLUB);
		HoldUp(iVictim)
