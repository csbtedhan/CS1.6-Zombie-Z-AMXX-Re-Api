...
new c_wpn_radius = 80
new c_wpn_bdamage = 2650
new c_wpn_bradius = 350
new c_wpn_velo = 1000
new c_wpn_bvelo = 650
new c_wpn_charge = 30
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_magicmg.txt"))
	{
		delete_file("script/custom_item/weapon_magicmg.txt")
		new File = fopen("script/custom_item/weapon_magicmg.txt", "a")
		...
		fprintf(File, "PROJECTILE_SKILL_DAMAGE %i^n", c_wpn_bdamage)
		fprintf(File, "PROJECTILE_SKILL_RADIUS %i^n", c_wpn_bradius)
		fprintf(File, "PROJECTILE_SKILL_VELOCITY %i^n", c_wpn_bvelo)
		fprintf(File, "PROJECTILE_CHARGE %i^n", c_wpn_charge)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_magicmg.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "PROJECTILE_SKILL_DAMAGE")) c_wpn_bdamage = str_to_num(value)
		else if(equal(key, "PROJECTILE_SKILL_RADIUS")) c_wpn_bradius = str_to_num(value)
		else if(equal(key, "PROJECTILE_SKILL_VELOCITY")) c_wpn_bvelo = str_to_num(value)
		else if(equal(key, "PROJECTILE_CHARGE")) c_wpn_charge = str_to_num(value)
...
public ShotBall(id, iMode)
{
	...
	velocity_by_aim(id, iMode ? c_wpn_bvelo : c_wpn_velo, vVelocity)
...
public BallImpact(Ent, id)
{
	...
	if(Damaging(pevAttacker, Ent, pOrigin, iMode ? float(c_wpn_bradius) : float(c_wpn_radius), iMode ? float(c_wpn_bdamage) : float(c_wpn_damage)) == 1 && !iMode && is_user_connected(pevAttacker))
	{
		...
		if(pev_valid(wpn) && Get_BitVar(g_HadWpn, pevAttacker) && pev(wpn, pev_iuser2) == 0)
		{
			...
			if(pev(wpn, pev_iuser4) >= c_wpn_charge)
			{
