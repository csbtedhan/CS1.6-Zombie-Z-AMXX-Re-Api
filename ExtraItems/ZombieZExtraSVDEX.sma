...
new c_wpn_damage = 450
new Float:c_wpn_speed = 0.3
new c_grenade_ammo = 10
new c_grenade_radius= 350
new c_grenade_damage = 2750
...
load_config()
{
	...
	if(!file_exists("script/custom_item/weapon_svdex.txt"))
	{
		...
		new File = fopen("script/custom_item/weapon_svdex.txt", "a")
		...
		fprintf(File, "GRENADE_RADIUS %i^n", c_grenade_radius)
		fprintf(File, "GRENADE_DAMAGE %i^n", c_grenade_damage)
		fprintf(File, "GRENADE_AMMO %i^n", c_grenade_ammo)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_svdex.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "GRENADE_RADIUS")) c_grenade_radius = str_to_num(value)
		else if(equal(key, "GRENADE_DAMAGE")) c_grenade_damage = str_to_num(value)
		else if(equal(key, "GRENADE_AMMO")) c_grenade_ammo = str_to_num(value)
	}
	fclose(file)
}
...
public plugin_init()
{
	...
	register_clcmd("weapon_svdex", "hook_command")
}
public hook_command(id)
...
public GiveBuffM4(id)
{
	...
	if(pev_valid(Ent)) 
	{
		...
		set_pev(Ent, pev_iuser3, c_grenade_ammo);
		
		set_pdata_int(id, 376+15, c_grenade_ammo)
	}
}
...
public fw_PrimaryAttack(Ent)
{
	...
	if(pev(Ent, pev_iuser4) && get_pdata_float(Ent, 46, 4) <= 0.0)
	{
		if(pev(Ent, pev_iuser3)> 0)
		{
			...
			set_pdata_int(Id, 376+15, pev(Ent, pev_iuser3))
		}
...
public GrenadeExplode(Ent, iPtd)
{
	...
	if(!cs_zb_is_zombie(pevAttacker))
	{
		...
		while((pEnt = engfunc(EngFunc_FindEntityInSphere, pEnt, vecOrigin, float(c_grenade_radius))) != 0 )
		{
			if(!pev_valid(pEnt))
				continue;
			...
			flDamage = float(c_grenade_damage) - floatmul(float(c_grenade_damage), floatdiv(flDist, float(c_grenade_radius)));
			
			if(isBreakable(pEnt) && flDamage <= 0.0) flDamage = floatdiv(float(c_grenade_damage), 2.0)
...
stock Set_WpnList(id)
{
	message_begin(MSG_ONE_UNRELIABLE, get_user_msgid("WeaponList"), _, id)
	write_string(Get_BitVar(g_HadWpn, id) ? "weapon_svdex" : weapon_svdex)
	write_byte(2)
	write_byte(90)
	write_byte(Get_BitVar(g_HadWpn, id) ? 15 : -1)
	write_byte(Get_BitVar(g_HadWpn, id) ? 10 : -1)
	write_byte(0)
	write_byte(1)
