...
new c_wpn_clip = 6
new c_wpn_damage = 52
new c_wpn_penetration = 1
new c_wpn_burntime = 3
new c_wpn_burndamage = 100
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_sapientia.txt"))
	{
		delete_file("script/custom_item/weapon_sapientia.txt")
		new File = fopen("script/custom_item/weapon_sapientia.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "CLIP_SIZE %i^n", c_wpn_clip)
		fprintf(File, "BASE_DAMAGE %i^n", c_wpn_damage)
		fprintf(File, "PENETRATION %i^n", c_wpn_penetration)
		fprintf(File, "BURN_DURATION %i^n", c_wpn_burntime)
		fprintf(File, "BURN_DAMAGE %i^n", c_wpn_burndamage)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_sapientia.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "PENETRATION")) c_wpn_penetration = str_to_num(value)
		else if(equal(key, "BURN_DURATION")) c_wpn_burntime = str_to_num(value)
		else if(equal(key, "BURN_DAMAGE")) c_wpn_burndamage = str_to_num(value)
	}
	fclose(file)
}
public plugin_init()
...
public RG_FireBullets3(pEntity, Float:vecSrc[3], Float:vecDirShooting[3], Float:vecSpread, Float:flDistance, iPenetration, iBulletType, iDamage, Float:flRangeModifier, iAttacker, bool:bPistol, shared_rand)
{
	if(!is_user_connected(iAttacker) || get_user_weapon(iAttacker) != CSW_DEAGLE && !Get_BitVar(g_HadWpn, iAttacker)) 
		return
		
	SetHookChainArg(6, ATYPE_INTEGER, c_wpn_penetration)
...
public MakeBurn(pevAttacker, pevVictim)
{
	...
	set_pev(pEnt, pev_dmgtime, get_gametime() + float(c_wpn_burntime));
}
public BurnThink(Ent)
{
	...
	{
		ExecuteHamB(Ham_TakeDamage, pevVictim, Ent, pevOwner, float(c_wpn_burndamage), DMG_BURN);
