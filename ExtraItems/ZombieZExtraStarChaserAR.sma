...
new c_wpn_star_interval = 1
new c_wpn_star_radius= 180
new c_wpn_star_damage = 420
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_starchaserar.txt"))
	{
		delete_file("script/custom_item/weapon_starchaserar.txt")
		new File = fopen("script/custom_item/weapon_starchaserar.txt", "a")
		...
		fprintf(File, "STAR_RADIUS %i^n", c_wpn_star_radius)
		fprintf(File, "STAR_DAMAGE %i^n", c_wpn_star_damage)
		fprintf(File, "STAR_INTERVAL %i^n", c_wpn_star_interval)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_starchaserar.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "STAR_RADIUS")) c_wpn_star_radius = str_to_num(value)
		else if(equal(key, "STAR_DAMAGE")) c_wpn_star_damage = str_to_num(value)
		else if(equal(key, "STAR_INTERVAL")) c_wpn_star_interval = str_to_num(value)
	}
	fclose(file)
}
public plugin_init()
...
{
	if(!is_user_connected(iAttacker) || !Get_BitVar(g_HadWpn, iAttacker) || get_user_weapon(iAttacker) != CSW_AUG) 
		return HC_CONTINUE;
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	...
	{
		flLastStar[invoker] = get_gametime() + float(c_wpn_star_interval);
		ShootStar(invoker)
...
public ShootStar(id)
{
	...
	Damaging(id, vTarget, float(c_wpn_star_radius), float(c_wpn_star_damage))
