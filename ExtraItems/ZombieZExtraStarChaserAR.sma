...
#define V_MODEL "models/zbz/wpn/v_starchaserar.mdl"
#define P_MODEL "models/zbz/wpn/p_starchaserar.mdl"
#define W_MODEL "models/zbz/wpn/w_primgroup1.mdl"

#define CSW_STARCHASERAR CSW_AUG
#define weapon_starchaserar "weapon_aug"

#define Get_BitVar(%1,%2) (%1 & (1 << (%2 & 31)))
#define Set_BitVar(%1,%2) %1 |= (1 << (%2 & 31))
#define UnSet_BitVar(%1,%2) %1 &= ~(1 << (%2 & 31))
new g_HadWpn, g_Event, g_Attack, Float:flLastStar[33]
...
new const WeaponSounds[][] = {
	"weapons/starchaserar-1.wav",
	"weapons/starchaserar-2.wav",
...
public plugin_init()
{
	register_plugin("[Zombie Z] Star Chaser AR", "1.0", "csbtedhan");
	...
	RegisterHam(Ham_Weapon_PrimaryAttack, weapon_starchaserar, "fw_PrimaryAttack_Post");
	...
	cs_register_custom_item("Star Chaser AR", 5000, 4, "GiveBUFFSG552", "starchaserar")
}
public fw_PrecacheEvent_Post(type, const name[])
	if(equal("events/aug.sc", name)) g_Event = get_orig_retval()
...
public message_DeathMsg()
{
	...
	if(Get_BitVar(g_HadWpn, killer) && equal(weapon, "aug"))
		set_msg_arg_string(4, "starchaserar")
...
public fw_TraceAttack(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	if(!is_user_connected(Attacker))
		return HAM_IGNORED	
	if(get_user_weapon(Attacker) != CSW_STARCHASERAR || !Get_BitVar(g_HadWpn, Attacker))
		return HAM_IGNORED
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, "models/w_aug.mdl"))
	{
		...
		{
			set_pev(weapon, pev_impulse, 2027) 
...
public fw_Item_PostFrame(ent)
{
	...
	static bpammo; bpammo = cs_get_user_bpammo(id, CSW_STARCHASERAR)
	...
	if(fInReload && flNextAttack <= 0.0)
	{
		...
		cs_set_user_bpammo(id, CSW_STARCHASERAR, bpammo - temp1)
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	if(!is_user_alive(id))
		return FMRES_IGNORED	
	if(get_user_weapon(id) == CSW_STARCHASERAR && Get_BitVar(g_HadWpn, id))
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (!is_user_connected(invoker))
		return FMRES_IGNORED	
	if(get_user_weapon(invoker) != CSW_STARCHASERAR || !Get_BitVar(g_HadWpn, invoker))
		return FMRES_IGNORED
	if(eventid != g_Event)
		return FMRES_IGNORED
	...
	if(flLastStar[invoker] <= get_gametime())
	{
		flLastStar[invoker] = get_gametime() + 1.0;
		ShootStar(invoker)
		engfunc(EngFunc_EmitAmbientSound, invoker, origin, WeaponSounds[1], 1.0, 0.6, 0, PITCH_NORM)
	} 
...
public ShootStar(id)
{
	...
	engfunc(EngFunc_EmitAmbientSound, 0, vTarget, WeaponSounds[2], 1.0, 0.6, 0, 100);
	Damaging(id, vTarget, 180.0, 420.0)
