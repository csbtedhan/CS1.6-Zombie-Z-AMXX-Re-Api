...
//STAB
new Float:c_stab_angle
new Float:c_stab_damage
new Float:c_stab_range
new Float:c_stab_kb
...
//SLASH SUMMONER
new Float:c_summon_angle
new Float:c_summon_damage
new Float:c_summon_range
new Float:c_summon_kb
...
//STAB SUMMONER
new Float:c_summonstab_damage
new Float:c_summonstab_range
new Float:c_summonstab_kb
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_summonknife.txt"))
	{
		delete_file("script/custom_item/weapon_summonknife.txt")
		new File = fopen("script/custom_item/weapon_summonknife.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "SUMMON_AMMO %i^n^n", c_max_ammo)
		...
		fprintf(File, "STAB_ANGLE %i^n", floatround(STAB_ANGLE))
		fprintf(File, "STAB_DAMAGE %i^n", floatround(STAB_DAMAGE))
		fprintf(File, "STAB_RANGE %i^n", floatround(STAB_RANGE))
		fprintf(File, "STAB_KNOCKBACK %i^n^n", floatround(STAB_KNOCKBACK))
		...
		fprintf(File, "SUMMON_SLASH_ANGLE %i^n", floatround(SMNERSLASH_ANGLE))
		fprintf(File, "SUMMON_SLASH_DAMAGE %i^n", floatround(SMNERSLASH_DAMAGE))
		fprintf(File, "SUMMON_SLASH_RANGE %i^n", floatround(SMNERSLASH_RANGE))
		fprintf(File, "SUMMON_SLASH_KNOCKBACK %i^n^n", floatround(SMNERSLASH_KNOCKBACK))
		
		fprintf(File, "SUMMON_STAB_DAMAGE %i^n", floatround(SMNERSTAB_DAMAGE))
		fprintf(File, "SUMMON_STAB_RANGE %i^n", floatround(SMNERSTAB_RANGE))
		fprintf(File, "SUMMON_STAB_KNOCKBACK %i^n^n", floatround(SMNERSTAB_KNOCKBACK))
		
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64], extra[64]
	file = fopen("script/custom_item/weapon_summonknife.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "STAB_DAMAGE")) c_stab_damage = str_to_float(value)
		else if(equal(key, "STAB_ANGLE")) c_stab_angle = str_to_float(value)
		else if(equal(key, "STAB_RANGE")) c_stab_range = str_to_float(value)
		else if(equal(key, "STAB_KNOCKBACK")) c_stab_kb = str_to_float(value)
		...
		else if(equal(key, "SUMMON_SLASH_DAMAGE")) c_summon_damage = str_to_float(value)
		else if(equal(key, "SUMMON_SLASH_ANGLE")) c_summon_angle = str_to_float(value)
		else if(equal(key, "SUMMON_SLASH_RANGE")) c_summon_range = str_to_float(value)
		else if(equal(key, "SUMMON_SLASH_KNOCKBACK")) c_summon_kb = str_to_float(value)
		else if(equal(key, "SUMMON_STAB_DAMAGE")) c_summonstab_damage = str_to_float(value)
		else if(equal(key, "SUMMON_STAB_RANGE")) c_summonstab_range = str_to_float(value)
		else if(equal(key, "SUMMON_STAB_KNOCKBACK")) c_summonstab_kb = str_to_float(value)
		else if(equal(key, "SUMMON_AMMO")) c_max_ammo = str_to_num(value)
	}
	fclose(file)
}
...
public client_PostThink(id)
{
	...
	if(g_iSummonerAttacking[id] && flSaskehHit && flSaskehHit < get_gametime())
	{
		if(g_iSummonerAttackMod[id] == 5)
		{
			...
			new pEntity = -1
			while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vecEnd, c_summonstab_range)) != 0)
			{
				...
				{
					ExecuteHamB(Ham_TakeDamage, pEntity, id, id, c_summonstab_damage, DMG_BULLET)
						
					Stock_Fake_KnockBack(id, pEntity, c_summonstab_kb)
					if(is_user_alive(pEntity)) SpawnBlood(vecEnd, get_pdata_int(pEntity,89), floatround(c_summonstab_damage/5.0))
				}
			}
		} else KnifeAttack_Setting(id, true, c_summon_range, c_summon_angle, c_summon_damage, c_summon_kb)
...
public SpecialHandler(id, iEnt, iButton)
{
	...
	if (get_pdata_float(id, 83) <= 0.0 && pev(iEnt, pev_iuser3) == 1)
	{
		...
		new iHitResult = KnifeAttack_Setting(id, 0, c_stab_range,c_stab_angle, c_stab_damage, c_stab_kb)
		switch (iHitResult)
		{
	...
	if(iButton & IN_ATTACK)
	{
		...
		switch(iState)
		{
			...
			case 2:
			{
				...
				new iHitResult = KnifeAttack_Setting(id, 0, c_slash_range, c_slash_angle, c_slash_damage, c_slash_kb)
				switch (iHitResult)
				{
					case RESULT_HIT_PLAYER : SendSound(id, CHAN_VOICE, weapon_sound[random_num(5,6)])
					case RESULT_HIT_WORLD : SendSound(id, CHAN_VOICE, weapon_sound[7])
...
public Explosions(iPtr, iOwner)
{
	...
	new pEntity = -1
	while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, fOrigin, c_charge_range)) != 0)
	{
		if(pev(pEntity, pev_takedamage) == DAMAGE_NO) continue
		...
		if(pev_valid(pEntity))
		{
			ExecuteHamB(Ham_TakeDamage, pEntity, iOwner, iOwner, c_charge_damage, DMG_BULLET)
				
			Stock_Fake_KnockBack(iOwner, pEntity, c_charge_kb)
			if(is_user_alive(pEntity)) SpawnBlood(fOrigin, get_pdata_int(pEntity,89), floatround(c_charge_damage/5.0))
		}
	}
}
