...
new c_wpn_damage = 95
new c_wpn_radius = 650
new Float:c_wpn_speed = 0.2
new c_bhole_duration = 6
new c_bhole_radius= 400
new c_bhole_damage = 180
new c_bhole_charge = 80
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_voidpistol.txt"))
	{
		delete_file("script/custom_item/weapon_voidpistol.txt")
		new File = fopen("script/custom_item/weapon_voidpistol.txt", "a")
		...
		fprintf(File, "SCAN_RADIUS %i^n", c_wpn_radius)
		fprintf(File, "BLACKHOLE_RADIUS %i^n", c_bhole_radius)
		fprintf(File, "BLACKHOLE_DAMAGE %i^n", c_bhole_damage)
		fprintf(File, "BLACKHOLE_DURATION %i^n", c_bhole_duration)
		fprintf(File, "BLACKHOLE_CHARGE %i^n", c_bhole_charge)
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_voidpistol.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "SCAN_RADIUS")) c_wpn_radius = str_to_num(value)
		else if(equal(key, "BLACKHOLE_RADIUS")) c_bhole_radius = str_to_num(value)
		else if(equal(key, "BLACKHOLE_DAMAGE")) c_bhole_damage = str_to_num(value)
		else if(equal(key, "BLACKHOLE_DURATION")) c_bhole_duration = str_to_num(value)
		else if(equal(key, "BLACKHOLE_CHARGE")) c_bhole_charge = str_to_num(value)
	}
	fclose(file)
}
...
{
	if(!is_user_connected(iAttacker) || !Get_BitVar(g_HadWpn, iAttacker) || get_user_weapon(iAttacker) != CSW_DEAGLE) 
		return HC_CONTINUE;
...
{	
	...
	if(Get_BitVar(g_Attack, iAttacker) && is_user_alive(iVictim) && cs_zb_is_zombie(iVictim))
	{
		...
		if(pev_valid(iWpn))
		{
			...
			if(pev(iWpn, pev_iuser1) >= c_bhole_charge && pev(iWpn, pev_iuser2) != 2)
			{
...
public fw_PrimaryAttack(Ent)
{
	...
	new isScanning = FindEnemy(Id, float(c_wpn_radius))
...
public fw_Item_PostFrame(ent)
{
	...
	if(get_user_button(id) & IN_ATTACK && get_pdata_float(ent, 46, 4) <= 0.0 && get_pdata_int(ent, 51, 4) > 0)
	{
		...
		if(isScanning)
		{
			...
			Set_BitVar(g_Attack,id)
			new iTotal = MakeBurstShot(id, float(c_wpn_radius), get_pdata_int(ent, 51, 4))
	...
	switch(iType)
	{
		...
		case 2: {
			if(pev(Ent, pev_sequence) != 1)
			{
				...
				set_pev(Ent, pev_dmgtime, get_gametime() + float(c_bhole_duration));
				set_pev(Ent, pev_fuser1, get_gametime() + float(c_bhole_duration/2));
			...
			{
				PullEnemyIntoBlackholes(vOrigin, float(c_bhole_radius))
				DoRadiusDamage(pevOwner, Ent, vOrigin, float(c_bhole_damage), float(c_bhole_radius))
			}
...
stock MakeBurstShot(id, Float:flRadius, iMax)
{
	...
	for(new x = 0; x < get_maxplayers(); x ++ )
	{
		...
		{
			rg_multidmg_clear();
			ExecuteHamB(Ham_TraceAttack, x, id, c_wpn_damage, vDir, tr, DMG_BULLET)
