...
new c_wpn_clip = 30
new c_wpn_damage = 70
new c_wpn_bdamage = 25
new c_wpn_radius = 200
new Float:c_wpn_speed
new c_bomb_radius
new c_bomb_damage
...
load_config()
{
	if(!file_exists("script/custom_item/weapon_wondercannon.txt"))
	{
		delete_file("script/custom_item/weapon_wondercannon.txt")
		new File = fopen("script/custom_item/weapon_wondercannon.txt", "a")
		
		fprintf(File, "// Config Generated by AMXX^n^n")
		fprintf(File, "CLIP_SIZE %i^n", c_wpn_clip)
		fprintf(File, "BASE_DAMAGE %i^n", c_wpn_damage)
		fprintf(File, "BASE_RADIUS %i^n", c_wpn_radius)
		fprintf(File, "BASE_EXPLOSION_DAMAGE %i^n", c_wpn_bdamage)
		fprintf(File, "FIRE_RATE %f^n", SPEED)
		fprintf(File, "BOMB_RADIUS %i^n", floatround(BOMB_RAD))
		fprintf(File, "BOMB_DAMAGE %i^n", floatround(BOMB_MAXDAMAGE))
		fclose(File)
	}
	new file, linedata[1024], key[512], value[64]
	file = fopen("script/custom_item/weapon_wondercannon.txt", "rt")
	...
	while (file && !feof(file))
	{
		...
		else if(equal(key, "BASE_RADIUS")) c_wpn_radius = str_to_num(value)
		else if(equal(key, "BASE_EXPLOSION_DAMAGE")) c_wpn_bdamage = str_to_num(value)
		else if(equal(key, "FIRE_RATE")) c_wpn_speed = str_to_float(value)
		else if(equal(key, "BOMB_RADIUS")) c_bomb_radius = str_to_num(value)
		else if(equal(key, "BOMB_DAMAGE")) c_bomb_damage = str_to_num(value)
	}
	fclose(file)
}
...
public fw_TraceAttack_Player(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	...
	SetHamParamFloat(3, float(c_wpn_damage))
	return HAM_HANDLED
}
...
public fw_Think(ent)
{
	...
	if(equal(Classname, "exp_null"))
	{
		...
		if(seq < 11)
		{
			...
			new Float:fDmg, Float:fFDmg = float(c_wpn_bdamage)
			...
			new pEntity = -1
			while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vEntOrigin, float(c_wpn_radius))) != 0)
			{
	...
	if(equal(Classname, WCB_PRIMCLASS) && pev(ent, pev_iuser1))
	{
		...
		if(iStat == 1 || iStat == 2)
		{
			...
			if(FindBombEnemy(id, vEntOrigin, float(c_wpn_radius)) > 0 || entity_range(id, ent) <= 70.0)
			{
				if(ent == g_iEnt[id][iSeq] && iSeq == g_iEntSeq[id]) 
					g_BombAct[id][iSeq] = 0
					
				bomb_explo(ent, id, vEntOrigin, float(c_bomb_radius), float(c_bomb_damage))
				return;
			}
		}
