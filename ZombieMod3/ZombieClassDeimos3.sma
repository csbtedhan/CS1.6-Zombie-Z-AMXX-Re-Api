	...
	if(CurButton & IN_RELOAD)
	{
		...
		if(g_flHealTime[id] <= get_gametime())
		{
			...
			if(cs_zb_get_zombie_class(id) == g_Classid)
			{
				...
				set_pev(id, pev_health,floatmin(float(cs_zb_get_user_maxhealth(id)), hp+2500.0))
			} else {
...
public BoltImpact(Ent, id)
{
	...
	for(new victim = 0; victim < 32; victim++)
	{
		...
		if(vector_distance(vOrigin, vecOrigin) <= 80.0 && isClearLine(vOrigin, vecOrigin))
		{
			...
			if(!g_FwResult)
			{
				if(!DropStripWeapon(victim, 1)) DropStripWeapon(victim, 2);
...
stock DropStripWeapon(id, sec)
{
	new item = get_pdata_cbase(id, 367 + sec)
	new droped = 0
	...
	while (item > 0)
	{
		...
		item = get_pdata_cbase(item, 42)
	}

	set_pdata_cbase(id, 367, -1)
